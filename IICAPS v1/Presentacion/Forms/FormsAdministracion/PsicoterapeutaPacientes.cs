...
{
    public partial class PsicoterapeutaPacientes : Form
    {

        private static PsicoterapeutaPacientes instance;
        ControlIicaps control;
        string ID_psicoterapeuta;
        public PsicoterapeutaPacientes(string psicoterapeuta)
        {
            InitializeComponent();
        ...
        public static PsicoterapeutaPacientes getInstance(string paciente)
        {
            if (instance != null)
            {
                return instance = new PsicoterapeutaPacientes(paciente);
            }else
            {
                return instance;
            }
        ...
        private void consultarToolStripMenuItem_Click(object sender, EventArgs e)
        {
             try
                {
                    String ID = dataGridView1.CurrentRow.Cells[0].Value.ToString();
                    ReporteDePaciente fa = new ReporteDePaciente(this.ID_psicoterapeuta);
                    fa.FormClosed += new FormClosedEventHandler(form_Closed);
                    fa.ShowDialog();
                }
                catch (Exception ex)
